...
export function MultiSelectValue({
  ...
  if (single && selectedValues.size > 0) {
    ...
    // Fall back to the raw value until its item registers its label, so a
    // pre-filled (editing) trigger never flashes blank on first render.
    return (
      <span className="min-w-0 overflow-hidden">
        {items.get(selected) ?? selected}
      </span>
    );
  }
...
export function MultiSelectItem({
  ...
  // useLayoutEffect (not useEffect) so item labels register before the first
  // paint — avoids a one-frame flash of raw ids in pre-filled triggers.
  useLayoutEffect(() => {
